"""WebSocket chat API for bidirectional text/voice communication with streaming support."""
import json
import logging
import time
import base64
...
async def chat_stream_websocket(websocket: WebSocket):
    """WebSocket endpoint for streaming voice/text chat.
    
    Optimized for low latency (<5s voice Q&A):
    - Immediate status feedback
    - Parallel LLM + emotion detection
    - Chunked audio delivery
    """
    await websocket.accept()
    logger.info("Stream WebSocket connection accepted: %s", websocket.client)
    ...
    async def _log_interaction(
        ...
        try:
            ...
            logger.warning("Stream interaction log failed: %s", e)
            await db.rollback()
    
    async def _process_voice_stream(audio_bytes: bytes, session_id: str):
        """Process voice input with streaming response for low latency."""
        start_time = time.time()
        
        # 1. Immediate status feedback (<100ms)
        ...
        # 2. ASR speech recognition (target <1s)
        try:
            asr_text = await transcribe(audio_bytes, language="zh")
            latency_asr = int((time.time() - start_time) * 1000)
            
            await websocket.send_json({
                "type": "asr_result",
                "text": asr_text,
                "latency_ms": latency_asr
            })
        except Exception as e:
            logger.error("ASR failed in stream: %s", e)
            await websocket.send_json({"type": "error", "message": "语音识别失败"})
            return
        ...
        # 4. Parallel: LLM generation + emotion detection
        async with async_session() as db:
            llm_task = asyncio.create_task(
                process_chat(asr_text, session_id, db, stream=False)
            )
            
            # Simple emotion detection (parallel)
            async def _detect_emotion(text: str) -> str:
                """Simple rule-based emotion detection."""
                emotion_keywords = {
                    'happy': ['高兴', '开心', '愉快', '太好了', '棒', '哈哈'],
                    'surprised': ['惊讶', '没想到', '竟然', '哇', '天哪'],
                    'grateful': ['感谢', '谢谢', '感恩', '辛苦了'],
                    'sad': ['难过', '伤心', '遗憾', '可惜'],
                    'curious': ['为什么', '怎么', '是什么', '告诉我'],
                }
            ...
            emotion_task = asyncio.create_task(_detect_emotion(asr_text))
            
            # Wait for LLM result
            result = await llm_task
            
            if not result.get("answer"):
                await websocket.send_json({"type": "error", "message": "AI 服务暂时不可用"})
                return
            ...
            # 8. TTS synthesis with chunked delivery
            try:
                ...
                if tts_result.audio_bytes:
                    ...
                    # Send phonemes for lip-sync
                    await websocket.send_json({
                        "type": "phonemes",
                        "data": tts_result.phoneme_timestamps
                    })
            except Exception as e:
                logger.error("TTS failed in stream: %s", e)
            ...
            await _log_interaction(
                ...
                source, sentiment_score, sentiment_label, total_latency
            )
    ...
    try:
        while True:
            ...
            if msg_type == "voice":
                ...
                await _process_voice_stream(audio_bytes, session_id)
                continue
            
            # ============== Text Path (with streaming) ==============
            ...
            async with async_session() as db:
                # Parallel: LLM + emotion
                llm_task = asyncio.create_task(
                    process_chat(question, session_id, db, stream=False)
                )
                ...
                emotion_task = asyncio.create_task(_detect_emotion(question))
        ...
        logger.info("Stream WebSocket disconnected: %s", websocket.client)
    except Exception as e:
        logger.error("Stream WebSocket unexpected error: %s", e)
        try:
...
async def chat_websocket(websocket: WebSocket):
    """WebSocket endpoint for chat (legacy non-streaming version).
